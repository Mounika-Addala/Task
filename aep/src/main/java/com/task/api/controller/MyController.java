...
public class MyController {
    ...
    @PutMapping("/updateAlertCount")
    public ResponseEntity<String> updateAlertCount(
            @RequestParam("_id") String _id,
            @RequestParam("newAlertCount") int newAlertCount) {
        try {
            apiService.updateAlertCount(_id, newAlertCount);
            return ResponseEntity.ok("Alert count updated successfully");
        } catch (Exception e) {
            e.printStackTrace();
            return ResponseEntity.status(500).body("Error updating alert count");
        }
    }
    ...
    public ResponseEntity<String> uploadExcel(@RequestParam("file") MultipartFile file) {
        try {
            ...
            if (!validationErrors.isEmpty()) {
                ...
                List<StationaryItem> entities = ExcelReader.readFromExcel(excelFile1, stationaryItemRepo);
