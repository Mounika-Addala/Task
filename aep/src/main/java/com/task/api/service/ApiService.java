...
public class ApiService {
    ...
    public void updateAlertCount(String _id, int newAlertCount) {
        Optional<StationaryItem> optionalItem = stationaryItemRepo.findById(_id);

        if (optionalItem.isPresent()) {
            StationaryItem item = optionalItem.get();
            item.setAlertCount(newAlertCount);
            stationaryItemRepo.save(item);
            System.out.println("Alert count updated successfully for item: " + _id);
        } else {
            System.out.println("Item with name '" + _id + "' not found in the database.");
        }
    }
